...
if ( class_exists( 'WXR_Importer' ) ) {
	...
	class Neovantage_WXR_Importer extends WXR_Importer {
			...
			foreach ( $node->childNodes as $child ) {
				...
				switch ( $child->tagName ) {
					...
					case 'wp:postmeta':
						...
						// Start PixelsPress edit.
						/*
						if ( ! empty( $meta_item ) ) {
							$meta[] = $meta_item;
						}
						*/
						// End PixelsPress edit.
						$meta[] = $meta_item;
		...
		protected function process_post_meta( $meta, $post_id, $post ) {
			...
			foreach ( $meta as $meta_item ) {
				...
				if ( $key ) {
					// Export meta straight from the DB so could have a serialized string.
					if ( ! $value ) {
					...
					// If post has a featured image, take note of this in case of remap.
		...
		protected function fetch_remote_file( $url, $post ) {
			...
			// Start PixelsPress edit.
			$headers = nc_wp_get_http( $url, $upload['file'] );
			// End PixelsPress edit.
			// Request failed.
		...
		protected function parse_term_node( $node, $type = 'term' ) {
			...
			foreach ( $node->childNodes as $child ) {
				...
				// PixelsPress edit: elseif condition is added ^.
			}
		...
		protected function process_term_meta( $meta, $term_id, $data ) {
			...
			foreach ( $meta as $meta_item ) {
				...
				// Get meta straight from the DB so could have a serialized string.
				$value = maybe_unserialize( $meta_item['value'] );
